...
async function experimentInit() {
  ...
  text = new visual.TextStim({
    ...
    font: 'Lucida Console',
    units: undefined, 
    pos: [0, 0], draggable: false, height: 0.05,  wrapWidth: 10, ori: 0,
    languageStyle: 'LTR',
...
var practice_sentenceMaxDurationReached;
var practice_sentenceMaxDuration;
...
function practice_sentenceRoutineBegin(snapshot) {
  return async function () {
    ...
    frameN = -1;
    continueRoutine = true; // until we're told otherwise
    practice_sentenceClock.reset();
    routineTimer.reset();
    practice_sentenceMaxDurationReached = false;
    // update component parameters for each repeat
    psychoJS.experiment.addData('practice_sentence.started', globalClock.getTime());
    practice_sentenceMaxDuration = null
    // keep track of which components have finished
...
var practice_qsMaxDurationReached;
var _qresp_p_allKeys;
var practice_qsMaxDuration;
...
function practice_qsRoutineBegin(snapshot) {
  return async function () {
    ...
    frameN = -1;
    continueRoutine = true; // until we're told otherwise
    practice_qsClock.reset();
    routineTimer.reset();
    practice_qsMaxDurationReached = false;
    // update component parameters for each repeat
...
var Start_experimentMaxDurationReached;
var _key_start_allKeys;
var Start_experimentMaxDuration;
